...
eye = [0, 0, 0.5]
targets = [[1.0, y, z] for y = -2:0.2:2, z = -2:0.2:2]

hits = [Bullet.raycast(sm, eye, target) for target in targets]
...
@testset "hit image" begin
  @test all(hit_image[1:10, 1:10] .== -1)
  @test all(hit_image[14:end, 10:end] .== 0)
  @test all(hit_image[1:10, 10:end] .== -1)
end
Bullet.disconnect()
